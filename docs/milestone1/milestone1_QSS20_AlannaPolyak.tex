...
\hl{Describe the final project you are pursuing}.

\begin{itemize}
\item Senior Thesis / Own Data: 
I am pursuing a project using my own data on the
mismatch between where the burden of neurological and neurodegenerative disease
falls and where the workforce capable of diagnosing and managing that disease exists. The unit of analysis is the country; the end goal is for the project to combine
...
the GBD and DHS extracts are in progress, and I am unsure to the extent I will be able to use them. The
project is Part~1 of a two-part design: the country-level
access gradient estimated in QSS 20 is intended to enter a separate household-level
analysis of informal caregiving reliance as a contextual predictor for QSS 45.

\end{itemize}

\section{Your project's main questions}

\hl{Write 1 paragraph on the main questions you hope to answer.}

I have three main questions that are all connected that I hope to answer. First, how large is the
...
distributional problem within countries; do countries with low
...
health system? I do not think that it will be possible to fully identify a causal effect of workforce density
on outcomes; the goal is to construct a defensible, reproducible measure of
the burden--capacity gap and to characterize its structure, which is a
prerequisite for the household-level question, but it is not its answer.

\section{Your project's data sources/relevant fields}

\hl{Give a 1 paragraph tentative summary of potentially relevant data and fields; you do not need to know the data exhaustively yet; this can just be based on codebook or reading in some sample data/viewing its head.}

The WHO/WFN Neurology Atlas is the main workforce source, and I extracted two tables
directly from the PDF. The first is the participating-country annex,
which yields 133 rows with fields \texttt{country}, \texttt{who\_region}, and
\texttt{wb\_income\_group}; this serves as the main aspect onto which the other
...
neurologists practice in the capital city, in other urban areas, and in rural
...
epilepsy, Parkinson disease, migraine) for all countries. The GBD file I currently have
is a global all-cause summary and is not usable at the country level; it
...
\hl{Create two visualizations using the skills you've gained from the Pandas and matplotlib modules. Embed them here.}
...
low-income countries against 7.1 in high-income countries, which is a 71-fold difference,
...
regression on this outcome will be badly skewed and likely need a log
or rank transformation.
\newpage

\begin{figure}[h!]
\centering
\includegraphics[width=0.8\textwidth]{fig1_workforce_by_income.png}
...
Figure~\ref{fig:two} helps to answer the second question for this project.
Reading across income groups, the share of countries reporting neurologists in
the capital city is almost flat, 95\% to 100\% everywhere, while the share
reporting any neurologist practicing in rural areas falls from 45\% in
...
This is the specific pattern the DHS geographic data is intended to quantify if I were to gain access.

\begin{figure}[h!]
\centering
\includegraphics[width=0.8\textwidth]{fig2.png}
\caption{Share of responding countries reporting that neurologists practice in
...
Read the report for \href{https://github.com/herbertfreeze/QSS20-X26/tree/main/finalproj_guidelines}{the provided example on your project option from Fall 2022} (for felony sentencing, read one of the two). \hl{Describe one lesson you take away from their report and one weakness of their approach upon which you will try to improve.}\\
...
The topic is unrelated to mine, but the structure is nearly
...
The lesson I take away is about their choice of comparison units: they never compare
...
\hl{Conduct a 1 paragraph pre-mortem on challenges your project may face -- e.g., measuring something, dealing with missing data, disentangling causality.}

There are a couple of challenges that are present. The first is measurement: ``neurological
...
comparisons are noisier than the medians suggest. The second is
missingness, since it is likely that it is not random: the countries that
failed to respond to the Atlas survey are likely those with the weakest health
...
panel will require decisions about which year to use the most and will
have broken coverage. Country-name harmonization across three sources with three
different naming conventions is a related issue. The fourth is
causality since workforce density,
disease burden, and national income are interconnected, income affects
...
workforce on outcomes would be uninterpretable. The project is as a result uses the burden--capacity gap as a construct to be
